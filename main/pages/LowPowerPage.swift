...
private func addLowPowerCard(_ panel: OpaquePointer?, _ verticalOffset: Int32, _ title: UnsafePointer<CChar>?) -> OpaquePointer? {
    let card = uiPixelPanelCreate(panel, 7, verticalOffset, 176, 42, uiPaper)
    let label = uiPixelLabel(card, title, swift_lvgl_font_montserrat_14(), uiInk)
    lv_obj_center(label)
    return card
}

func enterLowPowerPage() {
    lowPowerScreen = uiPixelScreenCreate("LOW POWER")
    guard let lowPowerScreen else { return }
    let panel = uiPixelPanelCreate(lowPowerScreen, 14, 54, 212, 190, uiPaper)
    lowPowerStatusLabel = uiPixelLabel(panel, swift_platform_power_initial_status(), swift_lvgl_font_montserrat_14(), uiInk)
...
func handleLowPowerPageKey(_ button: Int32, _ event: Int32) {
