...
| Wi-Fi | On-demand 2.4 GHz STA scan demo | `main/pages/WiFiPage.swift` | Scans only; it does not connect, store credentials, or validate antenna/RF performance |
| Bluetooth LE | On-demand non-connectable NimBLE advertising as `FoloPassport` | `main/pages/BluetoothPage.swift` | ESP32-C3 does not support Bluetooth Classic; radio range, coexistence, and power draw require device measurements |
| Low power | Two-second light sleep and five-second deep sleep, both with RTC timer wakeup | `main/pages/LowPowerPage.swift` | Deep sleep restarts the application; the current demo exposes RTC timer wake only |
