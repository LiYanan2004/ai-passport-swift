...
- 当用户要求删除不必要组件或直接进入某功能时，可以去掉主界面及其元素、直接载入目标功能界面；但必须保留 `ui_pixel` 主题体系（天空底色、草地、标题牌、吉祥物、墨色描边面板），让功能页使用 `UIPixelScreen`、`UIPixelPanel` 和 `UIPixelMascot` 保持统一的视觉效果。
- 默认情况下，在用户界面的右上角显示电量信息（读取 `Battery.shared.stateOfCharge`），除非开发者明确指定其它位置或明确不需要。读值为 `-1`（不可用）时显示不可用状态。
