...
- The `ui_pixel` theme (sky background, grass, title plate, mascot, ink-outlined panels) is part of the user interface. When trimming components or routing straight to a feature screen, keep the theme and build it with `UIPixelScreen`, `UIPixelPanel`, and `UIPixelMascot`.
- Show the battery level in the top-right corner of a user interface by default, unless the developer specifies a different placement or explicitly does not want it. Read it from `Battery.shared.stateOfCharge` (and `Battery.shared.voltageMillivolts` where useful); render it as a small battery indicator or percentage in the top-right area of the screen, and display an unavailable state for `-1`.
